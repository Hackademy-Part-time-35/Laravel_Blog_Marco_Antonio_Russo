<x-layout :$title>
    ...
    <main class="p-10" >
        ...
        <x-check-success />
        <form method="POST" enctype="multipart/form-data"  action="{{ route("books.store") }}" class="mt-10 px-5 lg:px-60 flex justify-center gap-5">
            @csrf
            ...
            <section class="mb-4 w-1/2  flex flex-col">
                ...
                <h1 id="titleEdit" contenteditable="true" class="text-3xl text-red-700"> {{old("title", "TITOLO")}}</h1>

                <h6 class="text-sm mb-5">

                    <em id="authorEdit"   contenteditable="true">
                        {{old("author")?old("author"):"autore"}}
                    </em>

                    <span> - </span>

                    <span id="dateEdit" contenteditable="true">
                        {{old("date")?old("date"):"anno"}}
                    </span>
                ...
                <p id="descEdit" contenteditable="true" class="mb-5">
                    {{old("description")?old("description"):"Trama"}}
                </p>


                <div class="text-end mt-20">
                    <x-rank :rank="0" />
                    <input id="starInput" name="rank" class="relative bottom-5 right-[39.3px] w-[106.81px] opacity-0" type="range" min="0" max="10" value="{{old("rank")?old("rank"):0}}">
                </div>


                <input class="hidden" id="bookTitle" type="text" name="title" value="TITOLO">
                <input class="hidden" id="bookAuthor" type="text" name="author">
                <input class="hidden" id="bookDate" type="text" name="date">
